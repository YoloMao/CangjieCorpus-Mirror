...
|              函数名          |           功能           |
| --------------------------- | ------------------------ |
| [assertCaughtUnexpectedE(String,String,String)](./unittest_package_api/unittest_package_functions.md#func-assertcaughtunexpectedestringstringstring) | 捕获的异常不符合预期，记录信息，抛出异常。 |
| [assertEqual\<T>(String, String, T, T): Unit where T <: Equatable\<T>](./unittest_package_api/unittest_package_functions.md#func-assertequaltstring-string-t-t-unit-where-t--equatablet) | 比较 `expected` 和 `actual` 值是否相等。若不等，直接抛出异常。 |
| [csv\<T>(String, Rune, Rune, Rune, Option\<Rune>, Option\<Array\<String>>, Array\<UInt64>, Array\<UInt64>, Bool)](./unittest_package_api/unittest_package_functions.md#func-csvtstring-rune-rune-rune-optionrune-optionarraystring-arrayuint64-arrayuint64-bool-where-t--serializablet) | 该函数可从 csv 文件中读取类型 T 的数据值，其中 T 必须可被序列化。该函数的返回值是参数化测试的一种参数源。 |
...
| [expectCaughtUnexpectedE(String,String,String)](./unittest_package_api/unittest_package_functions.md#func-expectcaughtunexpectedestringstringstring) | 捕获的异常不符合预期，记录信息，不抛出异常。 |
| [expectEqual\<T>(String, String, T, T): Unit where T <: Equatable\<T>](./unittest_package_api/unittest_package_functions.md#func-expectequaltstring-string-t-t-unit-where-t--equatablet) | 比较 `expected` 和 `actual` 值是否相等。记录比较结果，不抛出异常。 |
...
| [json\<T>(String)](./unittest_package_api/unittest_package_functions.md#func-jsontstring-where-t--serializablet) | 该函数可从 JSON 文件中读取类型 T 的数据值，其中 T 必须可被序列化。该函数的返回值是参数化测试的一种参数源。 |
| [tsv\<T>(String, Rune, Rune, Option\<Rune>, Option\<Array\<String>>, Array\<UInt64>, Array\<UInt64>, Bool)](./unittest_package_api/unittest_package_functions.md#func-tsvtstring-rune-rune-optionrune-optionarraystring-arrayuint64-arrayuint64-bool-where-t--serializablet) | 该函数可从 tsv 文件中读取类型 T 的数据值，其中 T 必须可被序列化。该函数的返回值是参数化测试的一种参数源。 |
...
| [Measurement](./unittest_package_api/unittest_package_interfaces.md#interface-measurement) | 在性能测试过程中可以收集和分析各种数据的接口。性能测试框架使用的特定实例由 `@Configure` 宏的 `measurement` 参数指定。 |
...
| [CartesianProductProcessor](./unittest_package_api/unittest_package_classes.md#class-cartesianproductprocessor) | 笛卡尔积处理器。 |
...
| [CsvStrategy](./unittest_package_api/unittest_package_classes.md#class-csvstrategy) | `DataStrategy` 对 CSV 数据格式的序列化实现。 |
|[DataStrategyProcessor](./unittest_package_api/unittest_package_classes.md#class-datastrategyprocessor)| 所有 [DataStrategy](../unittest_common/unittest_common_package_api/unittest_common_package_interfaces.md#interface-datastrategy) 组件的基类。该类的实例由 [@Strategy](../unittest_testmacro/unittest_testmacro_package_api/unittest_testmacro_package_macros.md#strategy-宏) 宏或成员函数创建。|
|[FlatMapProcessor](./unittest_package_api/unittest_package_classes.md#class-flatmapprocessor)| 对参数数据进行 [FlatMap](../collection/collection_package_api/collection_package_function.md#func-flatmapt-r-t---iterabler) 的处理器。 |
|[FlatMapStrategyProcessor](./unittest_package_api/unittest_package_classes.md#class-flatmapstrategyprocessor)| 对参数数据进行 [FlatMap](../collection/collection_package_api/collection_package_function.md#func-flatmapt-r-t---iterabler) 的处理器。 |
| [InputParameter](./unittest_package_api/unittest_package_classes.md#class-inputparameter) | 入参对象类型。 |
| [JsonStrategy](./unittest_package_api/unittest_package_classes.md#class-jsonstrategy) | `DataStrategy` 对 JSON 数据格式的序列化实现。 |
| [LazyCyclicNode](./unittest_package_api/unittest_package_classes.md#class-lazycyclicnode) | 用于在一个循环中一个接一个地推进类型擦除的内部惰性迭代器。 |
| [MapProcessor](./unittest_package_api/unittest_package_classes.md#class-mapprocessor) | 对参数数据进行 [Map](../collection/collection_package_api/collection_package_function.md#func-mapt-rt---r) 的处理器。 |
...
| [RandomDataProvider](./unittest_package_api/unittest_package_classes.md#class-randomdataprovider) | 使用随机数据生成的 DataProvider 接口的实现。 |
| [RandomDataShrinker](./unittest_package_api/unittest_package_classes.md#class-randomdatashrinker) | 使用随机数据生成的 DataShrinker 接口的实现。 |
| [RandomDataStrategy](./unittest_package_api/unittest_package_classes.md#class-randomdatastrategy) | 使用随机数据生成的 DataStrategy 接口的实现。 |
...
| [SerializableProvider](./unittest_package_api/unittest_package_classes.md#class-serializableprovider) | 获取序列化数据 [DataProvider](../unittest_common/unittest_common_package_api/unittest_common_package_interfaces.md#interface-dataprovider) 接口的实现。 |
| [SimpleProcessor](./unittest_package_api/unittest_package_classes.md#class-simpleprocessor) | 简单的数据策略处理器。对 [DataStrategyProcessor](./unittest_package_api/unittest_package_classes.md#class-datastrategyprocessor) 的一种实现。 |
...
|              结构体名          |           功能           |
| --------------------------- | ------------------------ |
| [BatchInputProvider](./unittest_package_api/unittest_package_structs.md#struct-batchinputprovider) | 输入提供程序，在执行之前在缓冲区中生成整个基准批次的输入。 |
| [BatchSizeOneInputProvider](./unittest_package_api/unittest_package_structs.md#struct-batchsizeoneinputprovider) | 基准输入提供程序，在每次执行基准之前生成输入。 |
| [GenerateEachInputProvider](./unittest_package_api/unittest_package_structs.md#struct-generateeachinputprovider) | 基准输入提供程序，在每次执行基准之前生成输入。 |
| [ImmutableInputProvider](./unittest_package_api/unittest_package_structs.md#struct-immutableinputprovider) | 最简单的输入提供程序，只需为基准测试的每次调用复制数据。适用于基准测试不会改变输入的情况。它在框架内默认使用。 |
