...
```cangjie
import compress.zlib.{CompressInputStream, DecompressInputStream, WrapType}
import net.http.*
import std.collection.ArrayList
import std.io.{ByteArrayStream, InputStream}
...
func readToEnd(input: InputStream): Array<Byte> {
    let buf = Array<Byte>(200, item: 0)
    let result = ArrayList<Byte>()
    var len = input.read(buf)
    while (len > 0) {
        result.appendAll(buf[0..len])
        len = input.read(buf)
    }
