...
在仓颉编程语言中，条件表达式分为 `if` 表达式和 `if-let` 表达式两种，它们的值与类型需要根据使用场景来确定。循环表达式有四种：`for-in` 表达式、`while` 表达式、`do-while` 表达式和 `while-let` 表达式，它们的类型都是 `Unit`、值为 `()`。其中 `if-let` 表达式和 `while-let` 表达式都与模式匹配相关，请参见[if-let 表达式](../enum_and_pattern_match/if_let.md)和[while-let 表达式](../enum_and_pattern_match/while_let.md)章节，本节只介绍以上提及的其他几种表达式。
...
关于区间类型的详细内容，请参阅基本数据类型[区间类型](../basic_data_type/range.md)章节。
